<template>
	<section
		class="row movies align-items-center justify-content-center red"
		v-if="!apiKey"
	>
		<div>Api key not found</div>
	</section>
	<section class="row movies" v-else>
		<div
		...
		>
			<div class="item">
				<a :href="getLink(item.imdbID)" target="_blank" class="image">
					...
					<span class="rating" :class="item.ratingClass">
						{{ item.imdbRatingText }}
					</span>
					<span
						class="year"
						:class="item.ratingClass"
						v-if="item.Year"
					>
						Year {{ item.Year }}
					</span>
				...
				<h2>
					<a :href="getLink(item.imdbID)" target="_blank">
						{{ item.Title }}
					</a>
				</h2>
			...
			await axios
				...
				.then((response) => {
					...
					this.movies.forEach((item, index) => {
						...
						if (this.movies[index]['Poster'] === 'N/A')
							this.movies[index][
								'Poster'
