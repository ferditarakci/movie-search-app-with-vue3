		...
		<div class="col-12 col-md-6">
			<form class="d-flex search" @submit.prevent="searchForm">
				<div class="label" ref="label">
					<input :placeholder="placeHolder" :value="searchText" @mousedown="searchFocus" type="text" class="form-control" autocomplete="off" required>
					<ul class="prev-search" ref="prevSearch">
						<li v-for="(item, index) in prevSearch" :key="index" @click="newSearch(item)">{{ item }}<i class="remove" @click="deleteSearch(index)">&times;</i></li>
					</ul>
				</div>
...
export default {
	...
	mounted() {
		...
		window.addEventListener("mousedown", (e) => {
			if (!(e.target.parentNode.className.indexOf("label") > -1 || e.target.parentNode.className == "prev-search") && e.target.className != "remove")
